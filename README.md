...
if(c.FM!==undefined){if(c.FM>0.20)add('critical','🚨','Sobrecarga orgánica severa','F/M='+c.FM.toFixed(3)+'. Reducir caudal urgente.');else if(c.FM>0.15)add('warning','⚠️','Sobrecarga orgánica','F/M='+c.FM.toFixed(3)+'. Sobre rango (0.05–0.15).');else if(c.FM<0.03)add('critical','🚨','Lodo envejecido severo','F/M='+c.FM.toFixed(3)+'. Posible bulking.');else if(c.FM<0.05)add('warning','⚠️','Lodo envejecido','F/M='+c.FM.toFixed(3)+'. Reducir purga.');else add('good','✅','F/M óptima','F/M='+c.FM.toFixed(3)+'. Equilibrio correcto.');}
